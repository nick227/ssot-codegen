// @generated
// Automated tests for Photo React hooks
...
describe('Photo React Hooks', () => {
  ...
  describe('usePhoto', () => {
    it('should fetch single photo by ID', async () => {
      const mockPhoto = { id: 'test-123', title: 'Test Photo' }
      mockAPI.get.mockResolvedValue(mockPhoto)
      
      const { result } = renderHook(() => usePhoto('test-123'), {
  ...
  describe('usePhotos', () => {
    it('should fetch list of photos', async () => {
      const mockList = {
        data: [
      ...
      const { result } = renderHook(() => usePhotos({ take: 20 }), {
        wrapper: createWrapper()
      })
    ...
    it('should accept query parameters', async () => {
      ...
      const { result } = renderHook(() => usePhotos(query), {
        wrapper: createWrapper()
      })
  ...
  describe('useInfinitePhotos', () => {
    it('should fetch pages of photos', async () => {
      const mockPage = {
  ...
  describe('useCreatePhoto', () => {
    it('should create photo', async () => {
      const newData = { title: 'New Photo', content: 'Test content' }
      const createdPhoto = { id: 'test-123', ...newData }
      mockAPI.create.mockResolvedValue(createdPhoto)
      ...
      expect(result.current.data).toEqual(createdPhoto)
    ...
    it('should call onSuccess callback', async () => {
      ...
      const { result } = renderHook(() => useCreatePhoto({ onSuccess }), {
        wrapper: createWrapper()
      })
    ...
    it('should call onError callback', async () => {
      ...
      const { result } = renderHook(() => useCreatePhoto({ onError }), {
        wrapper: createWrapper()
      })
  ...
  describe('useUpdatePhoto', () => {
    it('should update photo', async () => {
      const updatedPhoto = { id: 'test-123', title: 'Updated' }
      mockAPI.update.mockResolvedValue(updatedPhoto)
