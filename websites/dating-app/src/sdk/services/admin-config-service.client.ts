...
export class AdminConfigServiceClient {
  ...
  /**
   * getConfigStatus
   * GET /api/admin-config-service/config-status
   * 
   * @template TResponse - Response body type
   * @example
   * ```typescript
   * const result = await client.getConfigStatus<MyResponse>()
   * ```
   */
  async getConfigStatus<TResponse = unknown>(options?: QueryOptions): Promise<TResponse> {
    const response = await this.client.get<TResponse>(
      `/api/admin-config-service/config-status`,
      { signal: options?.signal }
    )
    return response.data
  }
