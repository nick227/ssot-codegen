...
export class DiscoveryServiceClient {
  ...
  /**
   * refreshQueue
   * POST /api/discovery-service/refresh-queue
   * 
   * @template TRequest - Request body type
   * @template TResponse - Response body type
   * @example
   * ```typescript
   * const result = await client.refreshQueue<MyRequest, MyResponse>({ field: 'value' })
   * ```
   */
  async refreshQueue<TRequest = Record<string, unknown>, TResponse = unknown>(data?: TRequest, options?: QueryOptions): Promise<TResponse> {
    const response = await this.client.post<TResponse>(
      `/api/discovery-service/refresh-queue`,
      data,
      { signal: options?.signal }
    )
    return response.data
  ...
  /**
   * and
   * POST /api/discovery-service/and
   * 
   * @template TRequest - Request body type
   * @template TResponse - Response body type
   * @example
   * ```typescript
   * const result = await client.and<MyRequest, MyResponse>({ field: 'value' })
   * ```
   */
  async and<TRequest = Record<string, unknown>, TResponse = unknown>(data?: TRequest, options?: QueryOptions): Promise<TResponse> {
    const response = await this.client.post<TResponse>(
      `/api/discovery-service/and`,
      data,
      { signal: options?.signal }
    )
    return response.data
