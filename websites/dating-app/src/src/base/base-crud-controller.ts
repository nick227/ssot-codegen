...
export interface CRUDService<T, CreateDTO, UpdateDTO, QueryDTO> {
  list(query: QueryDTO): Promise<{ data: T[]; meta: Record<string, unknown> }>
  ...
  createMany?(data: CreateDTO[] | unknown[]): Promise<{ count: number } | unknown>
  // updateMany accepts either UpdateDTO or Prisma's UpdateManyMutationInput (which excludes relations)
  updateMany?(where: Record<string, unknown>, data: UpdateDTO | unknown): Promise<{ count: number } | unknown>
  deleteMany?(where: Record<string, unknown>): Promise<{ count: number } | unknown>
  search?(params: QueryDTO): Promise<T[]>
  findBySlug?(slug: string): Promise<T | null>
  [key: string]: unknown  // Allow additional methods (getByX, listPublished, etc.)
}
...
export interface CRUDSchemas<CreateDTO, UpdateDTO, QueryDTO> {
  create: ZodSchema<CreateDTO>  // Zod schemas validate correctly at runtime
  update: ZodSchema<UpdateDTO>
  query: ZodSchema<QueryDTO>  // Zod schemas validate correctly at runtime even if types don't match exactly
}
...
export class BaseCRUDController<T, CreateDTO, UpdateDTO, QueryDTO> {
  ...
  private handleError(error: unknown, operation: string, context: Record<string, unknown>, res: Response): void {
  ...
  async create(req: Request, res: Response): Promise<Response> {
    try {
      ...
      const itemId = (item && typeof item === 'object' && 'id' in item) ? String((item as { id: unknown }).id) : 'unknown'
  ...
  async bulkCreate(req: Request, res: Response): Promise<Response> {
    try {
      ...
      const validatedData = body.data.map((item: unknown) => {
        try {
      ...
      const count = (result && typeof result === 'object' && 'count' in result) ? Number((result as { count: unknown }).count) : 0
...
export function createDomainMethodController<T>(
  serviceFn: (...args: unknown[]) => Promise<T | null>,
  config: {
    modelName: string
    methodName: string
    idType?: 'number' | 'string'
...
export function createVoidDomainMethodController(
  serviceFn: (...args: unknown[]) => Promise<void | unknown>,
  config: {
    modelName: string
    methodName: string
    idType?: 'number' | 'string'
...
export function createListMethodController<T>(
  serviceFn: (query: unknown) => Promise<{ data: T[]; meta: Record<string, unknown> }>,
  querySchema: ZodSchema<unknown> | z.ZodType<unknown>,
