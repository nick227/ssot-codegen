// @generated
// Automated tests for Block React hooks
...
describe('Block React Hooks', () => {
  ...
  describe('useBlock', () => {
    it('should fetch single block by ID', async () => {
      const mockBlock = { id: 'test-123', title: 'Test Block' }
      mockAPI.get.mockResolvedValue(mockBlock)
      
      const { result } = renderHook(() => useBlock('test-123'), {
  ...
  describe('useBlocks', () => {
    it('should fetch list of blocks', async () => {
      const mockList = {
        data: [
      ...
      const { result } = renderHook(() => useBlocks({ take: 20 }), {
        wrapper: createWrapper()
      })
    ...
    it('should accept query parameters', async () => {
      ...
      const { result } = renderHook(() => useBlocks(query), {
        wrapper: createWrapper()
      })
  ...
  describe('useInfiniteBlocks', () => {
    it('should fetch pages of blocks', async () => {
      const mockPage = {
      ...
      const { result } = renderHook(() => useInfiniteBlocks(), {
        wrapper: createWrapper()
      })
  ...
  describe('useCreateBlock', () => {
    it('should create block', async () => {
      const newData = { title: 'New Block', content: 'Test content' }
      const createdBlock = { id: 'test-123', ...newData }
      mockAPI.create.mockResolvedValue(createdBlock)
      ...
      expect(result.current.data).toEqual(createdBlock)
    ...
    it('should call onSuccess callback', async () => {
      ...
      const { result } = renderHook(() => useCreateBlock({ onSuccess }), {
        wrapper: createWrapper()
      })
    ...
    it('should call onError callback', async () => {
      ...
      const { result } = renderHook(() => useCreateBlock({ onError }), {
        wrapper: createWrapper()
      })
  ...
  describe('useUpdateBlock', () => {
    it('should update block', async () => {
      const updatedBlock = { id: 'test-123', title: 'Updated' }
      mockAPI.update.mockResolvedValue(updatedBlock)
