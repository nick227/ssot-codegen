...
/**
 * Generate Create DTO - Derived from Zod schema (single source of truth)
 * This eliminates duplication between DTOs and Zod schemas
 */
...
import type { ${model.name}CreateInput } from '@/validators/${modelKebab}/${modelKebab}.create.zod.js'

/**
 * Create DTO - Derived from Zod schema (single source of truth)
 * @see ${model.name}CreateSchema in validators
 */
export type ${model.name}CreateDTO = ${model.name}CreateInput
`
  } catch (error) {
...
/**
 * Generate Update DTO - Derived from Zod schema (single source of truth)
 * This eliminates duplication between DTOs and Zod schemas
 */
...
import type { ${model.name}UpdateInput } from '@/validators/${modelKebab}/${modelKebab}.update.zod.js'

/**
 * Update DTO - Derived from Zod schema (single source of truth)
 * @see ${model.name}UpdateSchema in validators
 */
export type ${model.name}UpdateDTO = ${model.name}UpdateInput
`
  } catch (error) {
    const message = error instanceof Error ? error.message : String(error)
...
export function generateQueryDTO(model: ParsedModel): string {
  ...
  // Array type for distinct (single field or array of fields)
  const distinctArrayType = distinctFields ? `Array<${distinctFields}>` : 'never'
  
    // Build imports section
    const enumImports = Array.from(enumTypes).length > 0
      ? `import { ${Array.from(enumTypes).join(', ')} } from '@prisma/client'\n\n`
      : ''
    
    return `// @generated
// This file is automatically generated. Do not edit manually.

${enumImports}import type { ${model.name}ReadDTO } from './${modelKebab}.read.dto.js'
