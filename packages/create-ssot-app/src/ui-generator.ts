  ...
  return (
    <div className="p-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        ...
        >
          <h2 className="text-xl font-semibold mb-2">📝 ${model.namePlural}</h2>
        ...
        columns={[${displayFields.map(f => `
          {
            ...
            cellRender: (value, row) => (
              ...
              >
                {${f.isRelation ? `value?.name || value?.title || '-'` : f.type === 'Boolean' ? `value ? '✓' : '○'` : 'value'}}
              </Link>
            )`}
          }`).join(',')
...
export function getUIDependencies(config: ProjectConfig): Record<string, string> {
  ...
  const deps: Record<string, string> = {
    ...
    '@ssot-ui/expressions': 'workspace:*'  // Expression system
  }
